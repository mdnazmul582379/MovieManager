...
async function handleMessage(message, env) {
  ...
  if (text === "➕ Add") {
    ...
    // Keep the single-movie and bulk examples in separate Telegram code blocks/messages.
    // This makes each JSON block independently copyable and keeps the explanatory OR text
    // outside the JSON/code formatting.
    const marker = "\n\nOR for multiple movies at once (send an array; every item uses the same full movie-data structure):\n";
    ...
    await tg(env, "sendMessage", {
      chat_id: message.chat.id,
      text: `<b>Add Movie</b>\nSend one movie JSON (each item must include "id"):\n\n<pre>${escHtml(singleExample)}</pre>`,
      parse_mode: "HTML",
    });

    await tg(env, "sendMessage", {
      chat_id: message.chat.id,
      text: `OR for multiple movies at once (send a JSON array; every item uses the same full movie-data structure):`,
      parse_mode: "HTML",
    });

    await tg(env, "sendMessage", {
      chat_id: message.chat.id,
      text: `<pre>${escHtml(bulkExample)}</pre>`,
      parse_mode: "HTML",
